function apiBuilderGet({arr,prefix,name} : IApiBuilderFromArray){
  this.get(prefix + name,  function (req : IReq, res  : IRes)  {
    ...
    let findedData = arr.find( (el : any) => {
...
function apiBuilderGetById({arr,prefix,name} : IApiBuilderFromArray){
  this.get(prefix +name +"/:id",(req : IReq,res : IRes) => {
    res.setHeader('Access-Control-Allow-Origin', '*');
